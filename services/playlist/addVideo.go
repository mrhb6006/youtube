...
import (
	...
	"youtube/models/playListVideo"
	"youtube/models/playlist"
	"youtube/pkg/response"
)

func AddVideoToPlayList(ctx *fiber.Ctx) error {
	baseErrCode := "073"
	request := playListType.AddVideoToPlayListRequest{}
	...
	exist, errStr, err = playListVideo.Repo.Exist(request.PlayListID, request.VideoID)
	if err != nil {
		return response.ErrorResponse(ctx, res, baseErrCode, "05", errStr, 500)
	}
	if exist {
		return response.ErrorResponse(ctx, res, baseErrCode, "06", "01", 200)
	}
	errStr, err = playListVideo.Repo.Insert(playListVideo.VideoPlayList{
